...
bool filter_size_eq(char *path, long value) {
  ...
  if (fstatat(AT_FDCWD, path, &file_stat,
              app_context.follow_links ? 0 : AT_SYMLINK_NOFOLLOW) == -1) {
...
bool filter_mime(char *path, char *mimetype) {
  ...
  if (S_ISLNK(file_stat.st_mode) && app_context.follow_links) {
    char *real_path = realpath(path, NULL);
    if (real_path == NULL) {
      fprintf(stderr, "Error while getting real path of %s", path);
      exit(1);
    }
    return filter_mime(real_path, mimetype);
  }
...
bool filter_ctc(char *path, char *ctc) {
  ...
  if (!S_ISDIR(path_stat.st_mode)) {
    if (S_ISLNK(path_stat.st_mode) && app_context.follow_links) {
      char *real_path = realpath(path, NULL);
      if (real_path == NULL) {
        fprintf(stderr, "Error while getting real path of %s", path);
        exit(1);
